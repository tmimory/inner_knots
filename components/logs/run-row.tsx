...
import { formatElapsed, formatRelative, truncate } from "@/lib/format";
...
export function RunRow({ run, characters }: RunRowProps) {
  ...
  return (
    <Link href={{ pathname: "/logs/[id]", params: { id: run.id } }} asChild>
      <Pressable
        role="link"
        className="gap-xs border-b-hairline border-border py-md transition-colors duration-fast active:bg-muted web:hover:bg-muted/subtle"
      >
        ...
        <View className="flex-row flex-wrap items-center gap-sm">
          ...
          <Text variant="meta" className="text-xs">
            {formatRelative(run.startedAt)} · {formatElapsed(run.startedAt, run.finishedAt)}
          </Text>
