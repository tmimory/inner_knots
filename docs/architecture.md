...
prompts/                 markdown prompt fragments (editable without touching code)
  ...
  st-petersburg/         variant-{thought-experiment,encounter}.md, terms.md,
                         payoff-{text,amount,forfeit}.md, toss-{won,forfeit}.md,
                         history.md, question.md
...
- **Run**: `id`, `puzzle: "trolley" | "prisoners-dilemma" | "st-petersburg" | "adventure"`, `config` (full puzzle configuration snapshot incl. roster and per-character run counts, a discriminated union on `puzzle`), `status`, `progress: { done, total }`, `startedAt`, `finishedAt`, `summary` (puzzle-specific reducer output: histogram counts, per-node frequencies). Prisoner's-dilemma payoffs are free text ("5 years", "walk free"), symmetric or per-player. The St. Petersburg config is a coin: two faces, each with a payoff and a flag saying whether landing that way ends the game, a maximum number of flips per game (up to 15), and a roster whose per-character run counts are games played. A payoff is one of three things — prose the voice says verbatim ("a sandwich"), money, paid on every flip that lands this way and optionally doubled for each flip after the first (the escalation the paradox is named for), or the forfeit of everything won so far — and the coin is put to a character under one of two framings: a thought experiment, or an encounter with a coin on the ground.
...
- **St. Petersburg** — `buildStPetersburgPrompt({ config, decisionStyle, flip?, history? })`. One prompt per turn, assembled the same way each time: the framing opens it (`variant-thought-experiment.md` or `variant-encounter.md`), `terms.md` states what the voice offers — each face's payoff clause, which the builder renders through that payoff's own kind of fragment (`payoff-text.md`, `payoff-amount.md`, `payoff-forfeit.md`) and hands in already worded, plus which face ends the game — `history.md` says which flip this is and lists the tosses so far with what each one won (`toss-won.md`, `toss-forfeit.md`, or the face's own prose) and, when the coin is priced, the running pot, `question.md` asks whether the character flips, and the shared decision instructions close it. The engine, not the model, tosses the coin — a random number after a `flip` answer — and does the arithmetic: an amount × 2^(flip − 1) when it doubles, minus the whole pot for a forfeit. The face and what it won join the history the next turn carries.
