...
/**
 * The full family list for one `fonts` key: the loaded family first, then its web
 * fallback stack. `mono` is already a stack, and a family with no stack of its own
 * falls back to the body serif rather than to the browser's default sans.
 */
function fontStack(key) {
  const stack = (fontFallbacks[key] ?? fontFallbacks.body).split(", ");
  return key === "mono" ? stack : [fonts[key], ...stack];
}
...
function colorBlock(colors, indent = "  ") {
  return Object.entries(colors)
    .map(
      ([key, hex]) =>
        `${indent}${cssVarNames.color(key)}: ${hexToRgbChannels(hex)};`,
    )
    .join("\n");
}
...
function staticBlock(indent = "  ") {
  ...
  // Driven by `fonts`, not by `fontFallbacks`: every family the app can ask for
  // must get a variable and a class, even if a stack is ever left out here.
  for (const key of Object.keys(fonts)) {
    lines.push(
      `${indent}${cssVarNames.font(key)}: ${fontStack(key).join(", ")};`,
    );
  }
  ...
  for (const [key, points] of Object.entries(easings)) {
    lines.push(
      `${indent}${cssVarNames.easing(key)}: cubic-bezier(${points.join(", ")});`,
    );
  }
  ...
  for (const group of scalarTokenGroups) {
    ...
    for (const [key, value] of Object.entries(group.record)) {
      lines.push(
        `${indent}${cssVarNames.scalar(group.prefix, key)}: ${value}${group.unit};`,
      );
    }
  }
...
const colorEntries = Object.keys(themeColors.scroll).map(
  (key) =>
    `    "${kebab(key)}": "rgb(var(${cssVarNames.color(key)}) / <alpha-value>)",`,
);
...
const fontFamilyEntries = Object.keys(fonts)
  .map((key) => `    "${key}": ${JSON.stringify(fontStack(key))},`)
  .join("\n");

const fontSizeEntries = Object.entries(fontSizes)
  .map(
    ([key, size]) =>
      `    "${key}": ["${size}px", { lineHeight: "${lineHeights[key]}px" }],`,
  )
  .join("\n");
...
console.log(
  "theme: wrote theme/global.css and theme/tailwind-tokens.cjs from theme/tokens.ts",
);
